...
const NewSales = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (result.status) {
        ...
        console.log('Error::new sales::fetchProducts::result', result.message)
      }
    } catch (error) {
      toast.error("Could not connect to backend");
      console.log('Error::new sales::fetchProducts', error)
    }
  }
  ...
  const handleSelectProductChange = (e) => {
    ...
    setselectedProductDetails({
      ...
      discount: 0,
      subtotal: (Number(product.p_price) * selectedProductDetails.qty).toFixed(2),
    })
  }
  const handleSelectQuantityChange = (e) => {
    const selectedProductQty = Number(e.target.value);
    setselectedProductDetails(prev => ({
      ...prev,
      qty: selectedProductQty,
      subtotal: (prev.price * selectedProductQty * ((100 - prev.discount) / 100)).toFixed(2)
    }))
  }
  const handleSelectDiscountChange = (e) => {
    const selectedProductDiscount = Number(e.target.value);
    setselectedProductDetails(prev => ({
      ...prev,
      discount: selectedProductDiscount,
      subtotal: (prev.price * prev.qty * ((100 - selectedProductDiscount) / 100)).toFixed(2)
    }))
  }
